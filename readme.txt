=== iContact, InfusionSoft form Popup ===
Contributors: cybernetikz
Donate link: 
Tags: icontactPopup,infusionsoftPopup,icontact,infusionsoft,infusion,popup,form popup,popup form,icontact popup,infusionsoft popup
Requires at least: 2.9.2
Tested up to: 4.2.4
Stable tag: 1.1

By this plugin you can override the default look and feel (stylesheet/css) of iContact and InfusionSoft web form.

== Description ==

By this plugin you can override the default look and feel (stylesheet/css) of iContact and InfusionSoft web form, also you can display them in your page/post as button using shortcode.

> <strong>PAID SUPPORT ($30/Hr, We ACCEPT PAYPAL )</strong>
>
> We assume most of the users would find it easy to install & use this plugin but those who needs <strong><em>PAID SUPPORT</em></strong> on any of the following:
>
> <strong>a. Get the plugin installed </strong>
>
> <strong>b. Custom feature, custom theme </strong>
>
> <strong>c. Auditing your wordpress theme & hosting environment</strong>
>
> <strong>d. Google analytics & webmaster tools support</strong>
>
> please send an email to this address <a href="mailto:support@cybernetikz.com">support@cybernetikz.com</a>

= Key Feature =

* Override the default look and feel (stylesheet/css) of iContact and InfusionSoft web form
* Display iContact and InfusionSoft web form by popup
* Also you can add custom web form (your own HTML form code)
* Use customize button image for iContact and InfusionSoft web form to display in page/post
* Display the form in page/post using the shortcode
* Support Wordpress multisite

Please visit our site <a href="http://www.cybernetikz.com/">www.cybernetikz.com/</a>

== Installation ==

1. Download the plugins and extract the files
2. Upload "icontact-infusionsoft-from-popup" to your wp-content/plugins/ directory
3. Activate the plugins through the 'Plugins' menu in Wordpress
4. Check the "Form Popup" Tab created by this plugins
5. After create a from copy paste the shortcode to page/post to display them as button

== Frequently Asked Questions ==

= How to use this plugins =

* After create a from copy paste the shortcode to page/post to display them as button

== Screenshots ==

1. Layout Add new form
2. Layout Manage form
3. Add shortcode to the page/post to display the form button
4. Sample iContact popup form layout
5. Sample InfusionSoft popup form layout

== Changelog ==

= 1.1 =
* PHP warning/notice fix
* Banner added in plugins settings pages

= 1.0 =
* First version.